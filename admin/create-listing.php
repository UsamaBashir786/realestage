...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_property'])) {
  ...
  $custom_rooms = trim($_POST['custom_rooms'] ?? '');

  if (empty($title)) $errors[] = "Title is required";
  if (empty($description)) $errors[] = "Description is required";
  if (empty($type)) $errors[] = "Type is required";
  if ($type === 'Custom' && empty($custom_type)) $errors[] = "Custom type is required";
  if (empty($status)) $errors[] = "Status is required";
  if (empty($price) || !is_numeric($price)) $errors[] = "Valid price is required";
  if (empty($area) || !is_numeric($area)) $errors[] = "Valid area is required";
  if ($rooms === 'Custom' && (empty($custom_rooms) || !is_numeric($custom_rooms))) $errors[] = "Valid custom rooms number is required";

  // Validate Location
  $address = trim($_POST['address'] ?? '');
  $county_state = trim($_POST['county_state'] ?? '');
  $type_location = trim($_POST['type_location'] ?? '');
  $city = trim($_POST['city'] ?? '');
  $neighborhood = trim($_POST['neighborhood'] ?? '');
  $zip = trim($_POST['zip'] ?? '');
  $country = trim($_POST['country'] ?? '');
  $custom_country = trim($_POST['custom_country'] ?? '');

  if (empty($address)) $errors[] = "Address is required";
  if (empty($county_state)) $errors[] = "County/State is required";
  if (empty($type_location)) $errors[] = "Location type is required";
  if (empty($city)) $errors[] = "City is required";
  if (empty($zip)) $errors[] = "Zip code is required";
  if ($country === 'Custom' && empty($custom_country)) $errors[] = "Custom country is required";
  ...
  $land_area_postfix = trim($_POST['land_area_postfix'] ?? '');
  $bedrooms = trim($_POST['bedrooms'] ?? '');
  $bathrooms = trim($_POST['bathrooms'] ?? '');
  $garages = trim($_POST['garages'] ?? '');
  $garages_size = trim($_POST['garages_size'] ?? '');
  $year_built = trim($_POST['year_built'] ?? '');
  $video_url = trim($_POST['video_url'] ?? '');
  $virtual_tour = trim($_POST['virtual_tour'] ?? '');

  if (empty($property_id)) $errors[] = "Property ID is required";
  if (!empty($area_size) && !is_numeric($area_size)) $errors[] = "Valid area size is required";
  if (!empty($bedrooms) && !is_numeric($bedrooms)) $errors[] = "Valid bedrooms number is required";
  if (!empty($bathrooms) && !is_numeric($bathrooms)) $errors[] = "Valid bathrooms number is required";
  if (!empty($year_built) && !is_numeric($year_built)) $errors[] = "Valid year built is required";

  // Validate Amenities
  $amenities = $_POST['amenities'] ?? [];
  $custom_amenities = trim($_POST['custom_amenities'] ?? '');

  // Validate Property Media
  $media_files = $_FILES['media_file'] ?? null;
  $attachment = $_FILES['attachment'] ?? null;
  $uploaded_files = [];

  if ($media_files && $media_files['error'][0] !== UPLOAD_ERR_NO_FILE) {
    $upload_dir = '../uploads/';
    if (!is_dir($upload_dir)) mkdir($upload_dir, 0777, true);

    foreach ($media_files['name'] as $key => $name) {
      if ($media_files['error'][$key] === UPLOAD_ERR_OK) {
        $ext = pathinfo($name, PATHINFO_EXTENSION);
        $filename = uniqid() . '.' . $ext;
        $target = $upload_dir . $filename;
        if (move_uploaded_file($media_files['tmp_name'][$key], $target)) {
          $uploaded_files[] = $filename;
        } else {
          $errors[] = "Failed to upload media file: $name";
        }
      }
    }
  }
  ...
  $plan_bedrooms = trim($_POST['plan_bedrooms'] ?? '');
  $plan_bathrooms = trim($_POST['plan_bathrooms'] ?? '');
  $plan_price = trim($_POST['plan_price'] ?? '');
  $plan_size_prefix = trim($_POST['plan_size_prefix'] ?? '');
  $plan_size = trim($_POST['plan_size'] ?? '');
  $plan_image = trim($_POST['plan_image'] ?? '');

  if (empty($plan_description)) $errors[] = "Plan description is required";
  if (!empty($plan_bedrooms) && !is_numeric($plan_bedrooms)) $errors[] = "Valid plan bedrooms number is required";
  if (!empty($plan_bathrooms) && !is_numeric($plan_bathrooms)) $errors[] = "Valid plan bathrooms number is required";
  if (!empty($plan_price) && !is_numeric($plan_price)) $errors[] = "Valid plan price is required";
  ...
  if (empty($errors)) {
    try {
      ...
      $final_type = $type === 'Custom' ? $custom_type : $type;
      $final_rooms = $rooms === 'Custom' ? $custom_rooms : $rooms;
      $final_country = $country === 'Custom' ? $custom_country : $country;

      $stmt = $conn->prepare("
                INSERT INTO properties (
                    title, description, type, status, price, area, rooms, 
                    address, county_state, type_location, city, neighborhood, zip, country,
                    property_id, area_size, size_prefix, land_area, land_area_postfix,
                    bedrooms, bathrooms, garages, garages_size, year_built, video_url, virtual_tour
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
      $stmt->execute([
        $title,
      ...
      // Insert amenities
      $all_amenities = array_merge($amenities, array_filter(array_map('trim', explode(',', $custom_amenities))));
      ...
      foreach ($uploaded_files as $file) {
        $stmt = $conn->prepare("INSERT INTO property_media (property_id, file_path) VALUES (?, ?)");
        $stmt->execute([$property_id, $file]);
      }

      // Insert floor plan
      $stmt = $conn->prepare("
                INSERT INTO floor_plans (
                    property_id, description, bedrooms, bathrooms, price, size_prefix, size, image
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
      $stmt->execute([
        $property_id,
      ...
      $conn->rollBack();
      $errors[] = "Failed to add property: " . $e->getMessage();
    }
  }
}
