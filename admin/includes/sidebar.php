<div class="dashboard__sidebar">
  ...
  <div class="dashboard__sidebar--inner">
    <ul class="sidebar__menu" id="accordionExample">
      ...
      <li class="sidebar__menu--items"><a class="sidebar__menu--link logout color-accent-2" href="../sign-up.php"><svg class="sidebar__menu--icon" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<script>
  ...
  document.querySelectorAll('.sidebar__dropdown--menu').forEach(dropdown => {
    dropdown.classList.remove('show');
  });

  // Reset dropdown links' expanded state
  document.querySelectorAll('.dropdown__link--active').forEach(link => {
    link.setAttribute('aria-expanded', 'false');
    link.classList.add('collapsed');
  });

  // Add active class based on current page
  const menuLinks = document.querySelectorAll('.sidebar__menu--link, .sidebar__dropdown--menu__link');

  menuLinks.forEach(link => {
    const href = link.getAttribute('href');

    if (href === pageName || href === currentPage) {
      // Direct match for the menu item
      link.classList.add('active');

      // If this is a submenu item, open its parent dropdown
      if (link.classList.contains('sidebar__dropdown--menu__link')) {
        const parentMenu = link.closest('.sidebar__dropdown--menu');
        const parentTrigger = document.querySelector(`[data-bs-target="#${parentMenu.id}"]`);
